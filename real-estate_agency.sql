...
Вариант 13:
    Создайте БД агентства недвижимости. Ориентировочные таблицы-составляющие: «Объекты недвижимости», «Типы объектов», «Типы сделок», «Покупатели», «Продавцы», «СДЕЛКИ».

Создать собственную базу данных по полученному варианту при этом выполнить следующие действия:
    - создать таблицы, указав ограничение NOT NULL, где необходимо;
    - придумать и создать ограничения (CHECK, UNIQUE);
    - создать индексы, синонимы, последовательности, обосновать их необ-ходимость;
    - внести данные в таблицы, где необходимо в качестве первичных клю-чей использовать псевдостолбцы созданных последовательностей.
...
1. Написать процедуру изменения мобильного номера продавца по указанной в качестве параметра фамилии. Контролировать, чтобы повторно не был вве-ден тот же номер.
2. Создать функцию, подсчитывающую количество сделок, совершенных потенциальными покупателями за текущий день. В вызывающую среду воз-вращать объекты недвижимости, участвующие в этих сделках.
...
6.	Написать анонимный PL/SQL-блок, в котором будут вызовы реали-зованных функций и процедур пакета с различными характерными значени-ями параметров для проверки правильности работы основных задач и обра-ботки исключительных ситуаций.
...
CURSOR article_cur
IS
    SELECT article_immov
        FROM immovables s, flows r
        WHERE TO_CHAR (transaction_date_and_time, 'DD.MM.YYYY') = TO_CHAR (SYSDATE, 'DD.MM.YYYY') and s.immovable_key = r.immovable_key;

counter NUMBER;

val_cf NUMBER;
val_nowDate DATE := SYSDATE;

PROCEDURE overload (VALUE IN DATE)
IS
today VARCHAR2(20) := TO_CHAR (VALUE, 'DD.MM.YYYY');
BEGIN
    DBMS_OUTPUT.PUT_LINE ('Сегодня: ' || today);
END;

PROCEDURE overload (VALUE IN NUMBER)
IS
today VARCHAR2(20) := TO_CHAR (VALUE, '999');
BEGIN
    SELECT COUNT (flows_key) INTO today
        FROM flows
        WHERE TO_CHAR (transaction_date_and_time, 'DD.MM.YYYY') = TO_CHAR (SYSDATE, 'DD.MM.YYYY');
    DBMS_OUTPUT.PUT_LINE ('За сегодня совершено '||today||' сделок.');
END;


BEGIN

overload (val_nowDate);
overload (val_cf);
    ...
    IF counter <> 0 THEN
        FOR var1 IN article_cur
        loop
        DBMS_OUTPUT.PUT_LINE ('Объект недвижимости участвовал в сделке сегодня: '||var1.article_immov);
        end loop;
    ELSE
...
BEGIN
    ...
    myPackage.CHANGE_TELNUM (vend_second_name => 'Karbisheva', new_vender_tel => '+375(11)111-11-11');
    myPackage.CHANGE_TELNUM (vend_second_name => 'qqqqqqqqqq', new_vender_tel => '+375(17)340-15-01');
    myPackage.CHANGE_TELNUM (vend_second_name => 'Karbisheva', new_vender_tel => '+375(17)340wwwwww');
...
2. Написать DDL-триггер, протоколирующий действия пользователей по созданию, изменению и удалению таблиц в схеме во вспомогательную табли-цу LOG2 в определенное время и запрещающий эти действия в другое время.
3. Написать системный триггер, добавляющий запись во вспомогатель-ную таблицу LOG3, когда пользователь подключается или отключается. В таблицу логов записывается имя пользователя (USER), тип активности (LOGON или LOGOFF), дата (SYSDATE), количество записей в основной таблице БД.
